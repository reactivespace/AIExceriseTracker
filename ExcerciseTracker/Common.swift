...
struct PlayerStats {
    ...
    mutating func getLastThrowType() -> ThrowType {
        guard let actionClassifier = try? ExcerciseModel_1(configuration: MLModelConfiguration()),
...
func warmUpVisionPipeline() {
    ...
    guard let image = #imageLiteral(resourceName: "Score1").cgImage,
          let detectorModel = try? ExcerciseModel_1(configuration: MLModelConfiguration()).model,
